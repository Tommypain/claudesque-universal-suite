import { useEffect, useRef, useState } from "react";
import { Ribbon } from "../ribbon/Ribbon";
import { RibbonGroup } from "../ribbon/RibbonGroup";
import { RibbonButton } from "../ribbon/RibbonButton";
import { useDocumentStore } from "../../store/useDocumentStore";

const exec = (cmd: string, val?: string) =>
  document.execCommand(cmd, false, val);

export function WriteApp() {
  const editorRef = useRef<HTMLDivElement>(null);
  const writeHtml = useDocumentStore((s) => s.writeHtml);
  const setWriteHtml = useDocumentStore((s) => s.setWriteHtml);
  const [margins, setMargins] = useState("96px 90px");
  const [pageW, setPageW] = useState(794);
  const [showTable, setShowTable] = useState(false);

  // Load html into editor when it changes externally (file open).
  useEffect(() => {
    const el = editorRef.current;
    if (el && el.innerHTML !== writeHtml) el.innerHTML = writeHtml;
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [writeHtml]);

  // Auto-save every 30s
  useEffect(() => {
    const t = setInterval(() => {
      if (editorRef.current)
        localStorage.setItem("octopus-write", editorRef.current.innerHTML);
    }, 30000);
    return () => clearInterval(t);
  }, []);

  const sync = () => {
    if (editorRef.current)
      useDocumentStore.setState({
        writeHtml: editorRef.current.innerHTML,
        dirty: true,
      });
  };

  const insertImage = () => {
    const input = document.createElement("input");
    input.type = "file";
    input.accept = "image/*";
    input.onchange = () => {
      const file = input.files?.[0];
      if (!file) return;
      const reader = new FileReader();
      reader.onload = () => {
        exec("insertImage", reader.result as string);
        sync();
      };
      reader.readAsDataURL(file);
    };
    input.click();
  };

  const insertTable = (r: number, c: number) => {
    let html = '<table border="1" style="border-collapse:collapse;width:100%">';
    for (let i = 0; i < r; i++) {
      html += "<tr>";
      for (let j = 0; j < c; j++) html += "<td style='padding:6px'>&nbsp;</td>";
      html += "</tr>";
    }
    html += "</table><p></p>";
    exec("insertHTML", html);
    setShowTable(false);
    sync();
  };

  const homeTab = (
    <>
      <RibbonGroup label="Font">
        <select
          className="oct-select"
          onChange={(e) => {
            exec("fontName", e.target.value);
            sync();
          }}
        >
          <option>Calibri</option>
          <option>Arial</option>
          <option>Georgia</option>
          <option>Times New Roman</option>
          <option>Courier New</option>
        </select>
        <select
          className="oct-select"
          defaultValue="3"
          onChange={(e) => {
            exec("fontSize", e.target.value);
            sync();
          }}
        >
          {[1, 2, 3, 4, 5, 6, 7].map((n) => (
            <option key={n} value={n}>
              {[8, 10, 12, 14, 18, 24, 36][n - 1]}
            </option>
          ))}
        </select>
      </RibbonGroup>
      <RibbonGroup label="Format">
        <RibbonButton icon={<b>B</b>} onClick={() => (exec("bold"), sync())} title="Bold" size="small" />
        <RibbonButton icon={<i>I</i>} onClick={() => (exec("italic"), sync())} title="Italic" size="small" />
        <RibbonButton icon={<u>U</u>} onClick={() => (exec("underline"), sync())} title="Underline" size="small" />
        <RibbonButton icon={<s>S</s>} onClick={() => (exec("strikeThrough"), sync())} title="Strikethrough" size="small" />
        <RibbonButton icon={<span>x₂</span>} onClick={() => (exec("subscript"), sync())} title="Subscript" size="small" />
        <RibbonButton icon={<span>x²</span>} onClick={() => (exec("superscript"), sync())} title="Superscript" size="small" />
        <label className="oct-rbtn small" title="Font color">
          <span className="ic">A</span>
          <input
            type="color"
            style={{ width: 20, height: 20, border: "none", background: "none" }}
            onChange={(e) => (exec("foreColor", e.target.value), sync())}
          />
        </label>
        <label className="oct-rbtn small" title="Highlight">
          <span className="ic">🖊</span>
          <input
            type="color"
            style={{ width: 20, height: 20, border: "none", background: "none" }}
            onChange={(e) => (exec("hiliteColor", e.target.value), sync())}
          />
        </label>
        <RibbonButton icon="⌫" onClick={() => (exec("removeFormat"), sync())} title="Clear formatting" size="small" />
      </RibbonGroup>
      <RibbonGroup label="Paragraph">
        <RibbonButton icon="⯇" onClick={() => (exec("justifyLeft"), sync())} title="Align left" size="small" />
        <RibbonButton icon="≡" onClick={() => (exec("justifyCenter"), sync())} title="Center" size="small" />
        <RibbonButton icon="⯈" onClick={() => (exec("justifyRight"), sync())} title="Align right" size="small" />
        <RibbonButton icon="☰" onClick={() => (exec("justifyFull"), sync())} title="Justify" size="small" />
        <RibbonButton icon="•" onClick={() => (exec("insertUnorderedList"), sync())} title="Bullet list" size="small" />
        <RibbonButton icon="1." onClick={() => (exec("insertOrderedList"), sync())} title="Numbered list" size="small" />
        <RibbonButton icon="→" onClick={() => (exec("indent"), sync())} title="Indent" size="small" />
        <RibbonButton icon="←" onClick={() => (exec("outdent"), sync())} title="Outdent" size="small" />
      </RibbonGroup>
      <RibbonGroup label="Styles">
        <select
          className="oct-select"
          defaultValue="p"
          onChange={(e) => {
            exec("formatBlock", e.target.value);
            sync();
          }}
        >
          <option value="p">Normal</option>
          <option value="h1">Heading 1</option>
          <option value="h2">Heading 2</option>
          <option value="h3">Heading 3</option>
          <option value="blockquote">Quote</option>
        </select>
      </RibbonGroup>
    </>
  );

  const insertTab = (
    <>
      <RibbonGroup label="Media">
        <RibbonButton icon="🖼" label="Image" onClick={insertImage} size="large" />
      </RibbonGroup>
      <RibbonGroup label="Tables">
        <div style={{ position: "relative" }}>
          <RibbonButton icon="▦" label="Table" onClick={() => setShowTable((v) => !v)} size="large" />
          {showTable && (
            <div className="oct-popover" style={{ top: "100%", left: 0 }}>
              <TablePicker onPick={insertTable} />
            </div>
          )}
        </div>
      </RibbonGroup>
      <RibbonGroup label="Links">
        <RibbonButton
          icon="🔗"
          label="Link"
          size="large"
          onClick={() => {
            const url = prompt("Enter URL");
            if (url) (exec("createLink", url), sync());
          }}
        />
        <RibbonButton
          icon="⤵"
          label="Page break"
          size="large"
          onClick={() => {
            exec("insertHTML", '<hr style="page-break-after:always">');
            sync();
          }}
        />
      </RibbonGroup>
    </>
  );

  const layoutTab = (
    <>
      <RibbonGroup label="Margins">
        <select
          className="oct-select"
          onChange={(e) => setMargins(e.target.value)}
          value={margins}
        >
          <option value="96px 90px">Normal</option>
          <option value="48px 48px">Narrow</option>
          <option value="96px 140px">Wide</option>
        </select>
      </RibbonGroup>
      <RibbonGroup label="Page">
        <select
          className="oct-select"
          onChange={(e) => setPageW(Number(e.target.value))}
          value={pageW}
        >
          <option value={794}>A4</option>
          <option value={816}>Letter</option>
        </select>
      </RibbonGroup>
    </>
  );

  return (
    <>
      <Ribbon
        tabs={[
          { id: "home", label: "Home", content: homeTab },
          { id: "insert", label: "Insert", content: insertTab },
          { id: "layout", label: "Layout", content: layoutTab },
        ]}
      />
      <div className="oct-workspace">
        <div
          ref={editorRef}
          className="oct-page"
          contentEditable
          suppressContentEditableWarning
          style={{ padding: margins, width: pageW }}
          onInput={sync}
          onBlur={sync}
        />
      </div>
    </>
  );
}

function TablePicker({ onPick }: { onPick: (r: number, c: number) => void }) {
  const [hover, setHover] = useState({ r: 0, c: 0 });
  const cells = [];
  for (let r = 0; r < 6; r++)
    for (let c = 0; c < 8; c++) {
      const on = r <= hover.r && c <= hover.c;
      cells.push(
        <div
          key={`${r}-${c}`}
          className={`oct-grid-cell${on ? " on" : ""}`}
          onMouseEnter={() => setHover({ r, c })}
          onMouseDown={(e) => {
            e.preventDefault();
            onPick(r + 1, c + 1);
          }}
        />,
      );
    }
  return (
    <>
      <div className="oct-grid-picker">{cells}</div>
      <div style={{ textAlign: "center", fontSize: 11, paddingBottom: 4 }}>
        {hover.r + 1} × {hover.c + 1}
      </div>
    </>
  );
}
