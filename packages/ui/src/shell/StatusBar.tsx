import { useDocumentStore, useAppStore, type AppId } from "@liberty/shared-hooks";
import { ZoomIn, ZoomOut } from "@liberty/icons";
...
export function StatusBar({ activeApp }: StatusBarProps) {
  const fileName = useDocumentStore((s: any) => s.fileName);
  const slides = useDocumentStore((s: any) => s.slides);
  const currentSlide = useDocumentStore((s: any) => s.currentSlide);
  const writeHtml = useDocumentStore((s: any) => s.writeHtml);
